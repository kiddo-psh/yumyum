# 냠냠코치 (NyamNyam Coach) — AGENTS.md

> Codex가 이 프로젝트를 작업할 때 반드시 읽어야 하는 컨텍스트 파일입니다.
...
Codex API를 통해 개인화된 코멘트와 식단 추천을 제공합니다.

**팀 구성:** 2인 팀
- Spring Boot 담당: 회원·식단·운동·커뮤니티·게이미피케이션 등 메인 비즈니스 로직 전체
- FastAPI 담당: AI 기능 전용 서버 (Codex API 연동, 칼로리 계산, 추세 분석)
...
| AI | Codex API (`Codex-haiku-4-5` 기본, `Codex-opus-4-5` RAG/Multi-Agent) |
| DB | MySQL 8, Redis 7 |
| 인프라 | Docker Compose, Nginx |

### Codex 모델 사용 기준
- **`Codex-haiku-4-5`**: 일반 AI 기능 (플랜 생성, 식단 추천, 루틴 조정 코멘트 등)
- **`Codex-opus-4-5`**: RAG 영양 채팅(F703), Multi-Agent 코칭(F704) — 크레딧 소모 주의
...
│       ├── services/
│       │   ├── claude_service.py      # Codex API 호출 단일 진입점
...
- [ ] `config.py`: `.env` 로딩, Codex 클라이언트 초기화
- [ ] `claude_service.py`: mock 모드 구현 (API 키 없는 개발 환경)
- [ ] `POST /ai/plan/generate`: BMR·TDEE 계산 + Codex 코멘트
...
async def generate_plan(data: OnboardingData):
    kcal = plan_service.calc_tdee(data)          # 공식 계산 (Codex 불필요)
    comment = await claude_service.generate(...)  # Codex API
...
- **모든 Codex API 호출은 `claude_service.py` 한 곳에서만** 이루어집니다.
- 엔드포인트 함수 안에 직접 `Codex.messages.create()`를 호출하지 마세요.
- mock 모드: `ENV=dev`일 때 Codex 호출 없이 더미 응답 반환 → API 키 없이 개발 가능
...
### plan_service.py — 칼로리 계산 (Codex 불필요)
```python
...
FastAPI 처리 순서:
  ...
  3. Codex API 프롬프트: 잔여 영양소 그램 수 포함, 한식 기반 1끼 분량 3개 추천
...
ANTHROPIC_API_KEY=sk-ant-...
FOOD_DB_API_KEY=...
ENV=dev                     # dev: mock 모드 / prod: 실제 Codex 호출
```
...
### FastAPI
- 모든 Codex 호출은 `claude_service.py` 경유
...
**Q. Codex API 키 없이 개발할 수 있나요?**
네. FastAPI의 `ENV=dev` 설정 시 `claude_service.py`가 mock 응답을 반환합니다.
Spring Boot와 Vue는 Codex 키와 무관하게 동작합니다.
